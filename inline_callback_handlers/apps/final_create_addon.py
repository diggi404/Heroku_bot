...
def final_create_addon(
    ...
    b1 = types.InlineKeyboardButton("✅", callback_data=f"yes create addon_{plan_id}")
...
Do you confirm creating this addon and link to the said app?
                          """,
        chat_id,
        msg_id,
        reply_markup=m,
...
def yes_create_addon(
    ...
    if (
        ...
        bot.edit_message_text("Kindly refetch your apps.", chat_id, msg_id)
        return
    new_msg = bot.edit_message_text("Creating addon...", chat_id, msg_id)
    app_id = addon_app_id_dict[chat_id]
    addon_name = app_details_dict["selected_addon"]
    plan_name = app_details_dict[plan_id][0]
    price = app_details_dict[plan_id][1]
    ...
    try:
        ...
        bot.edit_message_text(
            "Error creating addon. Try again.", chat_id, new_msg.message_id
        )
    else:
        if req.status_code == 201:
            ...
            m.add(b)
            bot.edit_message_text(
                f"Your addon <b>{addon_name}</b> with the <b>{plan_name}</b> plan has been created and link to the app <b>{app_name}</b> ✅.",
                chat_id,
                new_msg.message_id,
                parse_mode="HTML",
                reply_markup=m,
